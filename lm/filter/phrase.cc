...
public:
  ...
  void SetPhrase(detail::Vertex &from, detail::Vertex &to,
  ...
  Sentence Current() const { return *current_; }

  bool Empty() const { return current_ == last_; }
...
struct ArcGreater
    : public std::binary_function<const Arc *, const Arc *, bool> {
...
public:
  ...
  bool Empty() const { return incoming_.empty(); }
...
void BuildGraph(const Substrings &phrase, const std::vector<Hash> &hashes,
                detail::Vertex *const vertices, detail::Arc *free_arc) {
  using detail::Arc;
  using detail::Vertex;
  assert(!hashes.empty());
  ...
  // Phrases starting at or before the first word in the n-gram.
  {
    ...
    for (const Hash *word = first_word;; ++word, ++vertex) {
      ...
      if (!phrase.FindRight(hash, found))
        break;
      (free_arc++)->SetRight(*vertex, *found);
    }
  }
  ...
  for (const Hash *word_from = first_word + 1; word_from != &*hashes.end();
       ++word_from, ++vertex_from) {
    hash = 0;
    ...
    for (const Hash *word_to = word_from;; ++word_to, ++vertex_to) {
      ...
      if (!phrase.FindPhrase(hash, found))
        break;
      (free_arc++)->SetPhrase(*vertex_from, *vertex_to, *found);
    }
  }
...
ConditionCommon::ConditionCommon(const Substrings &substrings)
    : substrings_(substrings) {}

// Rest of the variables are temporaries anyway
ConditionCommon::ConditionCommon(const ConditionCommon &from)
    : substrings_(from.substrings_) {}
...
void Multiple::Evaluate(const StringPiece &line, Output &output) {
  detail::Vertex &last_vertex = MakeGraph();
  unsigned int lower = 0;
  while (true) {
    last_vertex.LowerBound(lower);
...
template void
Multiple::Evaluate<CountFormat::Multiple>(const StringPiece &line,
                                          CountFormat::Multiple &output);
template void
Multiple::Evaluate<ARPAFormat::Multiple>(const StringPiece &line,
                                         ARPAFormat::Multiple &output);
template void
Multiple::Evaluate<MultipleOutputBuffer>(const StringPiece &line,
                                         MultipleOutputBuffer &output);
