...
std::size_t hash_value(const MutablePiece &m) { return hash_value(m.behind); }
...
public:
  const char *Add(StringPiece str) {
    MutablePiece mut;
    mut.behind = str;
    std::pair<boost::unordered_set<MutablePiece>::iterator, bool> res(
        strs_.insert(mut));
    if (res.second) {
...
public:
  ...
  void Add(const std::vector<unsigned int> &sentences, StringPiece target) {
    ...
    for (std::vector<unsigned int>::const_iterator i(sentences.begin());
         i != sentences.end(); ++i) {
      boost::unordered_set<const char *> &vocab = vocab_[*i];
      for (std::vector<const char *>::const_iterator j = interns_.begin();
           j != interns_.end(); ++j) {
        vocab.insert(*j);
      }
    }
  ...
  void Print() const {
    ...
    for (std::vector<boost::unordered_set<const char *> >::const_iterator i =
             vocab_.begin();
         i != vocab_.end(); ++i) {
      for (boost::unordered_set<const char *>::const_iterator j = i->begin();
           j != i->end(); ++j) {
        out << *j << ' ';
      }
      out << '\n';
...
public:
  ...
  void AddSentence(StringPiece sentence, TargetWords &targets) {
    ...
    starts_.clear();
    starts_.push_back(0);
    for (util::TokenIter<util::AnyCharacter, true> i(sentence,
                                                     StringPiece("\0 \t", 3));
         i; ++i) {
    ...
    for (std::size_t i = 0; i < starts_.size() - 1; ++i) {
      ...
      for (std::size_t j = i + 1;
           j < std::min(starts_.size(), i + max_length_ + 1); ++j) {
        map_[util::MurmurHash64A(start, &canonical_[starts_[j]] - start - 1)]
            .push_back(sentence_id_);
      }
    }
    ++sentence_id_;
  }

  // Assumes single space-delimited phrase with no space at the beginning or
  // end.
  const std::vector<unsigned int> &Matches(StringPiece phrase) const {
    Map::const_iterator i =
        map_.find(util::MurmurHash64A(phrase.data(), phrase.size()));
