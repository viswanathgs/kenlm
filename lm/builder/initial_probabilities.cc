...
public:
  PruneNGramStream(const util::stream::ChainPosition &position,
      ...
      : current_(NULL, NGram<BuildingPayload>::OrderFromSize(
                           position.GetChain().EntrySize())),
        dest_(NULL, NGram<BuildingPayload>::OrderFromSize(
                        position.GetChain().EntrySize())),
        currentCount_(0), block_(position), specials_(specials) {
    StartBlock();
  }
  ...
  PruneNGramStream &operator++() {
    assert(block_);
    if (UTIL_UNLIKELY(current_.Order() == 1 &&
                      specials_.IsSpecial(*current_.begin())))
      dest_.NextInMemory();
...
public:
  ...
  void Run(const util::stream::ChainPosition &position) {
    for (util::stream::Link block_it(position); block_it; ++block_it) {
      if (pruning_) {
        const HashBufferEntry *in =
            static_cast<const HashBufferEntry *>(block_it->Get());
        const HashBufferEntry *end =
            static_cast<const HashBufferEntry *>(block_it->ValidEnd());

        // Just make it point to the beginning of the stream so it can be
        // overwritten With HashGamma values. Do not attempt to interpret the
        // values until set below.
        ...
        block_it->SetValidSize((block_it->ValidSize() * sizeof(HashGamma)) /
                               sizeof(HashBufferEntry));
      } else {
...
public:
  ...
  void Run(const util::stream::ChainPosition &output) {
    ...
    // Silly windows requires this workaround to just get an invalid pointer
    // when empty.
    void *const previous_raw =
        previous.empty() ? NULL : static_cast<void *>(&previous[0]);
    ...
    for (; in; ++out) {
      ...
      do {
        ...
        if (in->Value().CutoffCount() > 0)
          ++counts[std::min(in->Value().CutoffCount(),
      ...
      if (pruning_) {
        // If pruning is enabled the stream actually contains HashBufferEntry,
        // see InitialProbabilities(...), so add a hash value that identifies
        // the current ngram.
        static_cast<HashBufferEntry *>(&entry)->hash_value =
            util::MurmurHashNative(previous_raw, size);
      }
    }
    out.Poison();
  }
...
class MergeRight {
public:
  MergeRight(bool interpolate_unigrams,
             const util::stream::ChainPosition &from_adder,
             const Discount &discount, const SpecialVocab &specials)
      : interpolate_unigrams_(interpolate_unigrams), from_adder_(from_adder),
        discount_(discount), specials_(specials) {}

  // calculate the initial probability of each n-gram (before
  // order-interpolation) Run() gets invoked once for each order
  void Run(const util::stream::ChainPosition &primary) {
    ...
    if (grams->Order() == 1) {
      ...
      for (++grams; *grams->begin() != specials_.BOS(); ++grams) {
        grams->Value().uninterp.prob =
            discount_.Apply(grams->Value().count) / sums.denominator;
        grams->Value().uninterp.gamma = gamma_assign;
      }
    ...
    for (; grams; ++summed) {
      ...
      do {
        ...
        pay.uninterp.prob =
            discount_.Apply(grams->Value().UnmarkedCount()) / sums.denominator;
...
void InitialProbabilities(const InitialProbabilitiesConfig &config,
  ...
  for (size_t i = 0; i < primary.size(); ++i) {
    ...
    gamma_out[i] >>
        AddRight(discounts[i], second, prune_vocab || prune_thresholds[i] > 0);

    primary[i] >> MergeRight(config.interpolate_unigrams, gamma_out[i].Add(),
                             discounts[i], specials);

    // Don't bother with the OnlyGamma thread for something to discard.
    if (i)
      gamma_out[i] >> OnlyGamma(prune_vocab || prune_thresholds[i] > 0);
  }
}
