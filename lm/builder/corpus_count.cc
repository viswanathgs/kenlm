...
class DedupeEquals
    : public std::binary_function<const WordIndex *, const WordIndex *, bool> {
public:
...
public:
  Writer(std::size_t order, const util::stream::ChainPosition &position,
    ...
    assert(Dedupe::Size(position.GetChain().BlockSize() /
                        ...
                        kProbingMultiplier) == dedupe_mem_size);
    if (order == 1) {
  ...
  void Append(WordIndex word) {
    *(gram_.end() - 1) = word;
    ...
    bool found =
        dedupe_.FindOrInsert(DedupeEntry::Construct(gram_.begin()), at);
    if (found) {
      // Already present.
      NGram<BuildingPayload> already(at->key, gram_.Order());
      ++(already.Value().count);
      // Shift left by one.
      memmove(gram_.begin(), gram_.begin() + 1,
              sizeof(WordIndex) * (gram_.Order() - 1));
      return;
    }
    // Complete the write.
    gram_.Value().count = 1;
...
std::size_t CorpusCount::VocabUsage(std::size_t vocab_estimate) {
  return ngram::GrowableVocab<ngram::WriteUniqueWords>::MemUsage(
      vocab_estimate);
}

CorpusCount::CorpusCount(util::FilePiece &from, int vocab_write,
                         bool dynamic_vocab, uint64_t &token_count,
                         WordIndex &type_count, std::vector<bool> &prune_words,
                         const std::string &prune_vocab_filename,
                         std::size_t entries_per_block,
                         WarningAction disallowed_symbol)
    : from_(from), vocab_write_(vocab_write), dynamic_vocab_(dynamic_vocab),
      token_count_(token_count), type_count_(type_count),
...
void ComplainDisallowed(StringPiece word, WarningAction &action) {
  ...
  case SILENT:
    return;
  case COMPLAIN:
    std::cerr << "Warning: " << word
              << " appears in the input.  All instances of <s>, </s>, and "
                 "<unk> will be interpreted as whitespace."
              << std::endl;
    action = SILENT;
    return;
  case THROW_UP:
    UTIL_THROW(FormatLoadException,
               "Special word "
                   << word
                   << " is not allowed in the corpus.  I plan to support "
                      "models containing <unk> in the future.  Pass "
                      "--skip_symbols to convert these symbols to whitespace.");
  }
}
...
public:
  explicit VocabGiven(int fd) {
    util::MapRead(util::POPULATE_OR_READ, fd, 0,
                  util::CheckOverflow(util::SizeOrThrow(fd)), table_backing_);
    // Leave space for header with size.
    table_ = Table(static_cast<char *>(table_backing_.get()) + sizeof(uint64_t),
                   table_backing_.size() - sizeof(uint64_t));
    bos_ = FindOrInsert("<s>");
    eos_ = FindOrInsert("</s>");
  }
  ...
  WordIndex Index(const StringPiece &word) const { return FindOrInsert(word); }
...
private:
  ...
  typedef util::ProbingHashTable<ngram::ProbingVocabularyEntry,
                                 util::IdentityHash>
      Table;
  Table table_;
...
void CorpusCount::RunWithVocab(const util::stream::ChainPosition &position,
                               Vocab &vocab) {
  token_count_ = 0;
  type_count_ = 0;
