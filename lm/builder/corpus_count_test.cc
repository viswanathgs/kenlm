...
public:
  void Run(const util::stream::ChainPosition &position) {
    ...
    const char *v[] = {"<unk>",  "<s>",  "</s>", "looking", "on", "a",
                       "little", "more", "loin", "foo",     "bar"};
    WordIndex *w;
...
BOOST_AUTO_TEST_CASE(Short) {
  ...
  // loin[duplicate] </s>[duplicate] on[duplicate] foo little more loin </s> bar
  // </s> </s>
  ...
  CorpusCount counter(input_piece, vocab.get(), true, token_count, type_count,
                      prune_words, "", chain.BlockSize() / chain.EntrySize(),
                      SILENT);
