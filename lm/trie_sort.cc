...
// Proxy for an entry except there is some extra cruft between the entries. This
// is used to sort (n-1)-grams using the same memory as the sorted n-grams.
class PartialViewProxy {
public:
  PartialViewProxy() : attention_size_(0), inner_() {}

  PartialViewProxy(void *ptr, std::size_t block_size, util::FreePool &pool)
      : attention_size_(pool.ElementSize()), inner_(ptr, block_size),
        pool_(&pool) {}
...
void swap(PartialViewProxy first, PartialViewProxy second) {
  std::swap_ranges(reinterpret_cast<char *>(first.Data()),
                   reinterpret_cast<char *>(first.Data()) +
                       first.attention_size_,
...
FILE *DiskFlush(const void *mem_begin, const void *mem_end,
                const std::string &temp_prefix) {
  util::scoped_fd file(util::MakeTemp(temp_prefix));
  util::WriteOrThrow(file.get(), mem_begin,
                     (uint8_t *)mem_end - (uint8_t *)mem_begin);
  return util::FDOpenOrThrow(file);
}

FILE *WriteContextFile(uint8_t *begin, uint8_t *end,
                       const std::string &temp_prefix, std::size_t entry_size,
                       unsigned char order) {
  ...
  PartialIter context_begin(
      PartialViewProxy(begin + sizeof(WordIndex), entry_size, pool));
  PartialIter context_end(
      PartialViewProxy(end + sizeof(WordIndex), entry_size, pool));
  ...
  std::sort
      ...
      (context_begin, context_end,
       util::SizedCompare<EntryCompare, PartialViewProxy>(
           EntryCompare(order - 1)));

  util::scoped_FILE out(util::FMakeTemp(temp_prefix));

  // Write out to file and uniqueify at the same time.  Could have used
  // unique_copy if there was an appropriate OutputIterator.
...
struct ThrowCombine {
  void operator()(std::size_t entry_size, unsigned char order,
                  const void *first, const void *second, FILE * /*out*/) const {
...
FILE *MergeSortedFiles(FILE *first_file, FILE *second_file,
                       const std::string &temp_prefix, std::size_t weights_size,
                       unsigned char order, const Combine &combine) {
...
void RecordReader::Init(FILE *file, std::size_t entry_size) {
  ...
  UTIL_THROW_IF(!data_.get(), util::ErrnoException,
                "Failed to malloc read buffer");
  file_ = file;
  if (file) {
    rewind(file);
    remains_ = true;
...
void RecordReader::Overwrite(const void *start, std::size_t amount) {
  ...
  UTIL_THROW_IF(fseek(file_, internal - entry_size_, SEEK_CUR),
                util::ErrnoException, "Couldn't seek backwards for revision");
  util::WriteOrThrow(file_, start, amount);
  long forward = entry_size_ - internal - amount;
#if !defined(_WIN32) && !defined(_WIN64)
  if (forward)
#endif
    UTIL_THROW_IF(fseek(file_, forward, SEEK_CUR), util::ErrnoException,
                  "Couldn't seek forwards past revision");
}
...
SortedFiles::SortedFiles(const Config &config, util::FilePiece &f,
                         std::vector<uint64_t> &counts, size_t buffer,
  ...
  unigram_.reset(util::MakeTemp(file_prefix));
  {
    ...
    util::scoped_mmap unigram_mmap(
        util::MapZeroedWrite(unigram_.get(), size_out), size_out);
    Read1Grams(f, counts[0], vocab,
               reinterpret_cast<ProbBackoff *>(unigram_mmap.get()), warn);
  ...
  for (unsigned int order = 2; order < counts.size(); ++order) {
    buffer_use = std::max<size_t>(
        buffer_use,
        static_cast<size_t>((sizeof(WordIndex) * order + 2 * sizeof(float)) *
                            counts[order - 1]));
  }
  buffer_use = std::max<size_t>(
      buffer_use,
      static_cast<size_t>((sizeof(WordIndex) * counts.size() + sizeof(float)) *
                          counts.back()));
  ...
  if (!mem.get())
    UTIL_THROW(util::ErrnoException,
               "malloc failed for sort buffer size " << buffer);

  for (unsigned char order = 2; order <= counts.size(); ++order) {
    ConvertToSorted(f, vocab, counts, file_prefix, order, warn, mem.get(),
                    buffer);
  }
  ReadEnd(f);
}
...
public:
  ...
  ~Closer() {
    for (std::deque<FILE *>::iterator i = files_.begin(); i != files_.end();
         ++i) {
...
void SortedFiles::ConvertToSorted(util::FilePiece &f,
                                  const SortedVocabulary &vocab,
                                  const std::vector<uint64_t> &counts,
                                  const std::string &file_prefix,
                                  unsigned char order, PositiveProbWarn &warn,
                                  void *mem, std::size_t mem_size) {
  ...
  const size_t weights_size =
      sizeof(float) + ((order == counts.size()) ? 0 : sizeof(float));
  ...
  for (std::size_t batch = 0, done = 0; done < count; ++batch) {
    ...
    if (order == counts.size()) {
      for (; out != out_end; out += entry_size) {
        std::reverse_iterator<WordIndex *> it(
            reinterpret_cast<WordIndex *>(out) + order);
        ReadNGram(f, order, vocab, it,
                  *reinterpret_cast<Prob *>(out + words_size), warn);
      }
    } else {
      for (; out != out_end; out += entry_size) {
        std::reverse_iterator<WordIndex *> it(
            reinterpret_cast<WordIndex *>(out) + order);
        ReadNGram(f, order, vocab, it,
                  *reinterpret_cast<ProbBackoff *>(out + words_size), warn);
      }
    }
    ...
    contexts.push_back(
        WriteContextFile(begin, out_end, file_prefix, entry_size, order));
  ...
  while (files.size() > 1) {
    files.push_back(MergeSortedFiles(files[0], files[1], file_prefix,
                                     weights_size, order, ThrowCombine()));
    files_closer.PopFront();
    files_closer.PopFront();
    contexts.push_back(MergeSortedFiles(contexts[0], contexts[1], file_prefix,
                                        0, order - 1, FirstCombine()));
