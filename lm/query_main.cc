...
void Usage(const char *name) {
  std::cerr << "KenLM was compiled with maximum order " << KENLM_MAX_ORDER
            << ".\n"
               "Usage: "
            << name
            << " [-b] [-n] [-w] [-s] lm_file\n"
               ...
               "-l lazy|populate|read|parallel: Load lazily, with populate, or "
               "malloc+read\n"
               "The default loading method is populate on Linux and read on "
               "others.\n\n"
               "Each word in the output is formatted as:\n"
               "  word=vocab_id ngram_length log10(p(word|context))\n"
               "where ngram_length is the length of n-gram matched.  A "
               "vocab_id of 0 indicates\n"
               "the unknown word. Sentence-level output includes log10 "
               "probability of the\n"
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    if (RecognizeBinary(file, model_type)) {
      std::cerr << "This binary file contains "
                << lm::ngram::kModelNames[model_type] << "." << std::endl;
      switch (model_type) {
      case PROBING:
        Query<lm::ngram::ProbingModel>(file, config, sentence_context, printer);
        break;
      case REST_PROBING:
        Query<lm::ngram::RestProbingModel>(file, config, sentence_context,
                                           printer);
        break;
      case TRIE:
      ...
      Query<lm::np::Model, lm::ngram::QueryPrinter>(model, sentence_context,
                                                    printer);
      Query<lm::np::Model, lm::ngram::QueryPrinter>(model, sentence_context,
                                                    printer);
#endif
