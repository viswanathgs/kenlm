...
VocabFileReader::VocabFileReader(const int fd, const size_t model_num,
                                 uint64_t offset)
    : hash_value_(0), current_index_(0), eof_(false), model_num_(model_num),
      file_piece_(util::DupOrThrow(fd)) {
  word_ = file_piece_.ReadLine('\0');
  UTIL_THROW_IF(word_ != "<unk>", FormatLoadException,
...
VocabFileReader &VocabFileReader::operator++() {
  ...
  UTIL_THROW_IF(hash_value_ < prev_hash_value, FormatLoadException,
                ": word index not monotonically increasing."
                    << " model_num: " << model_num_ << " prev hash: "
                    << prev_hash_value << " new hash: " << hash_value_);
...
class CompareFiles {
public:
  bool operator()(const VocabFileReader *x, const VocabFileReader *y) {
...
WordIndex MergeVocab(util::FixedArray<int> &files, UniversalVocab &vocab,
  ...
  while (!heap.empty()) {
    ...
    vocab.InsertUniversalIdx(top_vocab_file->ModelNum(),
                             top_vocab_file->CurrentIndex(), global_index);
