...
// gcc 4.6 complains about uninitialized when sort code is generated for a
// 4-byte POD.  But that sort code is never used.
...
struct InitialExtensionCompare {
  bool operator()(const void *first, const void *second) const {
    return reinterpret_cast<const InitialExtension *>(first)->ext <
           reinterpret_cast<const InitialExtension *>(second)->ext;
  }
};
...
public:
  InstanceMatch(Matrix &backoffs_out, const WordIndex correct)
      : seen_(std::numeric_limits<WordIndex>::max()), backoffs_(backoffs_out),
        correct_(correct), correct_from_(1),
        correct_ln_prob_(std::numeric_limits<float>::quiet_NaN()) {}

  void MatchedBackoff(ModelIndex model, uint8_t order, float ln_backoff) {
    backoffs_(model, order - 1) = ln_backoff;
  }

  // We only want the highest-order matches, which are the first to be exited
  // for a given word.
  void Exit(const InitialExtension &from, util::stream::Stream &out) {
    if (from.ext.word == seen_)
      return;
    seen_ = from.ext.word;
...
private:
  // What's the last word we've seen?  Used to act only on exiting the longest
  // match.
  WordIndex seen_;
...
public:
  void Register(InstanceMatch &context) { registered_.push_back(&context); }

  void MatchedBackoff(ModelIndex model, uint8_t order, float ln_backoff) {
    for (std::vector<InstanceMatch *>::iterator i = registered_.begin();
         i != registered_.end(); ++i)
      (*i)->MatchedBackoff(model, order, ln_backoff);
  }

  void Exit(InitialExtension &from, util::stream::Stream &out,
...
// Map from n-gram hash to contexts in the tuning data.  TODO: probing hash
// table?
...
public:
  JointOrderCallback(std::size_t model, std::size_t full_order_minus_1,
                     ContextMap &contexts, util::stream::Stream &out,
                     const InstanceMatch *base_instance)
      : full_order_minus_1_(full_order_minus_1), contexts_(contexts), out_(out),
  ...
  void Exit(std::size_t order_minus_1, void *data) {
    ...
    if (order_minus_1 != full_order_minus_1_) {
      ...
      ContextMap::iterator i = contexts_.find(util::MurmurHashNative(
          gram.begin(), gram.Order() * sizeof(WordIndex)));
      if (UTIL_UNLIKELY(i != contexts_.end())) {
        i->second.MatchedBackoff(ext_.ext.model, gram.Order(),
                                 gram.Value().backoff * M_LN10);
      }
    }
    // Match the context of the n-gram to indicate it's an extension.
    ContextMap::iterator i = contexts_.find(
        util::MurmurHashNative(data, order_minus_1 * sizeof(WordIndex)));
    if (UTIL_UNLIKELY(i != contexts_.end())) {
...
public:
  ...
  void Run(const util::stream::ChainPosition &position) {
    ...
    for (; stream; ++stream) {
      ...
      out_.segment(previous, word - previous) =
          Vector::Constant(word - previous, unk);
      out_(word) = stream->Value().prob * M_LN10;
      // backoffs are used by JointOrderCallback.
      previous = word + 1;
    }
    out_.segment(previous, out_.rows() - previous) =
        Vector::Constant(out_.rows() - previous, unk);
  }
...
// Read tuning data into an array of vocab ids.  The vocab ids are agreed with
// MergeVocab.
...
public:
  IdentifyTuning(int tuning_file, std::vector<WordIndex> &out) : indices_(out) {
    ...
    for (util::FilePiece f(tuning_file); f.ReadLineOrEOF(line);) {
      for (util::TokenIter<util::BoolCharacter, true> word(line, util::kSpaces);
           word; ++word) {
        UTIL_THROW_IF(*word == "<s>" || *word == "</s>", FormatLoadException,
                      "Illegal word in tuning data: " << *word);
        words_[util::MurmurHashNative(word->data(), word->size())].push_back(
            counter++);
      }
  ...
  void Add(WordIndex id, const StringPiece &str) {
    boost::unordered_map<uint64_t, std::vector<std::size_t> >::iterator i =
        words_.find(util::MurmurHashNative(str.data(), str.size()));
    if (i != words_.end()) {
      for (std::vector<std::size_t>::iterator j = i->second.begin();
           j != i->second.end(); ++j) {
        indices_[*j] = id;
      }
    }
...
public:
  explicit ExtensionsFirstIteration(std::size_t instances, std::size_t models,
                                    std::size_t max_order,
                                    util::stream::Chain &extension_input,
                                    const util::stream::SortConfig &config)
      : backoffs_by_instance_(new std::vector<Matrix>(instances)),
        sort_(extension_input, config) {
    // Initialize all the backoff matrices to zeros.
    for (std::vector<Matrix>::iterator i = backoffs_by_instance_->begin();
         i != backoffs_by_instance_->end(); ++i) {
  ...
  // Get the backoff all the way to unigram for a particular tuning instance and
  // model.
...
private:
  class ApplyBackoffs {
  public:
    explicit ApplyBackoffs(
        boost::shared_ptr<std::vector<Matrix> > backoffs_by_instance)
    ...
    void Run(const util::stream::ChainPosition &position) {
      ...
      for (util::stream::Stream stream(position); stream; ++stream) {
        InitialExtension &ini =
            *reinterpret_cast<InitialExtension *>(stream.Get());
        assert(ini.order >
               1); // If it's an extension, it should be higher than a unigram.
        if (ini.order != max_order) {
          ini.ext.ln_prob +=
              backoffs[ini.ext.instance](ini.ext.model, ini.order - 1);
        }
      }
    }
  ...
  // This is a shared pointer so that ApplyBackoffs can run after this class is
  // gone.
  boost::shared_ptr<std::vector<Matrix> > backoffs_by_instance_;
...
Instances::Instances(int tune_file, const std::vector<StringPiece> &model_names,
                     const InstancesConfig &config)
    : temp_prefix_(config.sort.temp_prefix) {
  // All the memory from stack variables here should go away before merge sort
  // of the instances.
  {
    ...
    for (std::vector<StringPiece>::const_iterator i = model_names.begin();
         i != model_names.end(); ++i) {
      models.push_back(*i);
    ...
    // Setup the initial extensions storage: a chain going to a sort with a
    // stream in the middle for writing.
    util::stream::Chain extensions_chain(util::stream::ChainConfig(
        sizeof(InitialExtension), 2, config.extension_write_chain_mem));
    util::stream::Stream extensions_write(extensions_chain.Add());
    extensions_first_.reset(
        new ExtensionsFirstIteration(tuning_words.size(), model_names.size(),
                                     max_order, extensions_chain, config.sort));
    ...
    util::FixedArray<InstanceMatch> instances(tuning_words.size());
    {
      ...
      for (std::size_t i = 0; i < tuning_words.size(); ++i) {
        instances.push_back(boost::ref(extensions_first_->WriteBackoffs(i)),
                            tuning_words[i]);
        for (std::size_t j = 0; j < context.size(); ++j) {
          cmap[util::MurmurHashNative(&context[j],
                                      sizeof(WordIndex) * (context.size() - j))]
              .Register(instances.back());
        }
    ...
    for (std::size_t m = 0; m < models.size(); ++m) {
      std::cerr << "Processing model " << m << '/' << models.size() << ": "
                << model_names[m] << std::endl;
      ...
      for (std::size_t i = 0; i < models[m].Order(); ++i) {
        ...
        chains.push_back(
            util::stream::ChainConfig(NGram<ProbBackoff>::TotalSize(i + 1), 2,
                                      config.model_read_chain_mem));
      }
      ...
      // Send extensions into extensions_first_ and give data to the instances
      // about backoffs/extensions.
      chains >> JointOrderCallback(m, models[m].Order() - 1, cmap,
                                   extensions_write, instances.begin());

      chains >> util::stream::kRecycle;
      chains.Wait(true);
      neg_ln_correct_sum_(m) = 0.0;
      for (InstanceMatch *i = instances.begin(); i != instances.end(); ++i) {
        neg_ln_correct_sum_(m) -=
            i->FinishModel(m, ln_unigrams_(i->Correct(), m));
        ln_backoffs_(i - instances.begin(), m) =
            extensions_first_->FullBackoff(i - instances.begin(), m);
      }
      ln_unigrams_(bos_, m) =
          0; // Does not matter as long as it does not produce nans since
             // tune_derivatives will overwrite the output.
    }
...
void Instances::ReadExtensions(util::stream::Chain &on) {
  if (extensions_first_.get()) {
    // Lazy sort and save a sorted copy to disk.  TODO: cut down on record size
    // by stripping out order information.
    extensions_first_->Output(on);
    extensions_first_
        .reset(); // Relevant data will continue to live in workers.
    extensions_subsequent_.reset(
        new util::stream::FileBuffer(util::MakeTemp(temp_prefix_)));
