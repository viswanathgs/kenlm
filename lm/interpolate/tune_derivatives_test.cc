...
public:
  MockInstances()
      : chain_(util::stream::ChainConfig(ReadExtensionsEntrySize(), 2, 100)),
        write_(chain_.Add()) {
    extensions_subsequent_.reset(
        new util::stream::FileBuffer(util::MakeTemp("/tmp/")));
...
BOOST_AUTO_TEST_CASE(Small) {
...

  {
    ...
    Matrix unigrams(4, 2);
    unigrams << 0.1, 0.6, 0.4, 0.3, 0.5, 0.1,
        // <s>
        1.0, 1.0;
  ...
  // We'll suppose correct has WordIndex 1, which backs off in model 0, and
  // matches in model 1
  ...
  // p_I(x | context)
  Vector p_I(3);
  p_I << pow(0.1 * 0.2, 0.9) * pow(0.6 * 0.4, 1.2),
  ...
  expected_hessian(1, 0) -=
      (p_I(0) * log(0.1 * 0.2) + p_I(1) * log(0.4 * 0.2) +
       p_I(2) * log(model_0_word_2)) *
      (p_I(0) * log(0.6 * 0.4) + p_I(1) * log(model_1_word_1) +
       p_I(2) * log(0.1 * 0.4));
