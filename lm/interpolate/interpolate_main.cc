...
#pragma GCC diagnostic ignored                                                 \
    "-Wpragmas" // Older gcc doesn't have "-Wunused-local-typedefs" and
                // complains.
...
namespace {
void MungeWeightArgs(int argc, char *argv[],
  ...
  for (char **i = argv + 1; i < argv + argc; ++i) {
    ...
    if (starts_with(arg, "-w") || starts_with(arg, "--w")) {
      inside_weights = i;
    } else if (inside_weights && arg.size() >= 2 && arg[0] == '-' &&
...
int main(int argc, char *argv[]) {
  try {
    ...
    options.add_options()("help,h", po::bool_switch(),
        ...
        "model,m",
        po::value<std::vector<std::string> >(&input_models)
            ->multitoken()
            ->required(),
        "Models to interpolate, which must be in KenLM intermediate format.  "
        "The intermediate format can be generated using the --intermediate "
        "argument to lmplz.")(
        "weight,w",
        po::value<std::vector<float> >(&pipe_config.lambdas)->multitoken(),
        "Interpolation weights")(
        "tuning,t", po::value<std::string>(&tuning_file),
        "File to tune on: a text file with one sentence per line")(
        "just_tune", po::bool_switch(), "Tune and print weights then quit")(
        "temp_prefix,T",
        po::value<std::string>(&pipe_config.sort.temp_prefix)
            ->default_value("/tmp/lm"),
        "Temporary file prefix")(
        "memory,S",
        lm::SizeOption(pipe_config.sort.total_memory,
                       util::GuessPhysicalMemory() ? "50%" : "1G"),
        "Sorting memory: this is a very rough guide")(
        "sort_block", lm::SizeOption(pipe_config.sort.buffer_size, "64M"),
        "Block size");
    po::variables_map vm;
    ...
    po::store(po::parse_command_line((int)munged_args.size(),
                                     &*munged_args.begin(), options),
              vm);
    ...
    instances_config.extension_write_chain_mem =
        instances_config.sort.total_memory;
    instances_config.lazy_memory = instances_config.sort.total_memory;

    if (pipe_config.lambdas.empty() && tuning_file.empty()) {
      std::cerr << "Provide a tuning file with -t xor weights with -w."
                << std::endl;
      return 1;
    }
    if (!pipe_config.lambdas.empty() && !tuning_file.empty()) {
    ...
    if (!tuning_file.empty()) {
      ...
      for (std::vector<std::string>::const_iterator i = input_models.begin();
           i != input_models.end(); ++i) {
        model_names.push_back(*i);
      }
      lm::interpolate::TuneWeights(util::OpenReadOrThrow(tuning_file.c_str()),
                                   model_names, instances_config,
                                   pipe_config.lambdas);

      std::cerr << "Final weights:";
      std::ostream &to = vm["just_tune"].as<bool>() ? std::cout : std::cerr;
      for (std::vector<float>::const_iterator i = pipe_config.lambdas.begin();
           i != pipe_config.lambdas.end(); ++i) {
        to << ' ' << *i;
      }
      to << std::endl;
    }
    ...
    if (pipe_config.lambdas.size() != input_models.size()) {
      std::cerr << "Number of models (" << input_models.size()
                << ") should match the number of weights ("
                << pipe_config.lambdas.size() << ")." << std::endl;
      return 1;
    }
