...
Accum Derivatives(Instances &in, const Vector &weights, Vector &gradient,
                  Matrix &hessian) {
  ...
  // Even -inf doesn't work for <s> because weights can be negative.  Manually
  // set it to zero.
  ...
  util::stream::Chain chain(
      util::stream::ChainConfig(in.ReadExtensionsEntrySize(), 2, 64 << 20));
  ...
  for (InstanceIndex n = 0; n < in.NumInstances(); ++n) {
    ...
    // Loop over words within an instance for which extension exists.  An
    // extension happens when any model matches more than a unigram in the
    // tuning instance.
    while (extensions && extensions->instance == n) {
      ...
      // Calculate ln_p_i_full(i) = ln p_i(word | context) by filling in
      // unigrams then overwriting with extensions.
      ln_p_i_full = ln_p_i_backed;
      // Loop over all models that have an extension for the same word namely
      // p_i(word | context) matches at least a bigram.
      for (;
           extensions && extensions->word == word && extensions->instance == n;
           ++extensions) {
        ln_p_i_full(extensions->model) = extensions->ln_prob;
      }

      // This is the weighted product of probabilities.  In other words,
      // p_I(word | context) * Z(context) = exp(\sum_i w_i * p_i(word |
      // context)).
      ...
      full_cross.noalias() += weighted * ln_p_i_full -
                              weighted_uni(word) *
                                  weighted_backoffs /* we'll divide by Z_context
                                                       later to form B_I */
                                  * in.LNUnigrams().row(word).transpose();
      ...
      hessian_missing_Z_context.noalias() +=
          ...
          // Presumed unigrams.  Z_epsilon * weighted_backoffs will turn into
          // B_I once all of this is divided by Z_context.
          - weighted_uni(word) * weighted_backoffs * ln_p_i_backed *
                ln_p_i_backed.transpose();
    }

    Accum Z_context =
        weighted_backoffs *
            (Z_epsilon -
             unnormalized_sum_x_p_I)   // Back off and unnormalize the unigrams
                                       // for which there is no extension.
    ...
    // This is the gradient term for this instance except for -log p_i(w_n |
    // w_1^{n-1}) which was accounted for as part of neg_correct_sum_.
    // full_cross(i) is \sum_{all x} p_I(x | context) log p_i(x | context)
    // Prior terms excluded dividing by Z_context because it wasn't known at the
    // time.
    ...
    // There's one missing term here, which is independent of context and done
    // at the end.
    hessian.noalias() +=
        // First term of Hessian, assuming all models back off to unigram.
        B_I * (convolve + convolve.transpose() +
               in.LNBackoffs(n).transpose() * in.LNBackoffs(n))
        // Error in the first term, correcting from unigram to full
        // probabilities.
  ...
  for (Matrix::Index x = 0; x < weighted_uni.rows(); ++x) {
    ...
    // TODO can this be optimized?  It's summing over the entire vocab which
    // should be a matrix operation.
    hessian.noalias() += sum_B_I * weighted_uni(x) / Z_epsilon *
                         in.LNUnigrams().row(x).transpose() *
                         in.LNUnigrams().row(x);
  }
  return exp((in.CorrectGradientTerm().dot(weights) + sum_ln_Z_context) /
             static_cast<double>(in.NumInstances()));
}
