...
public:
  ...
  void Enter() { entry_ = stream_->Value().backoff; }
...
struct PtrGreater
    : public std::binary_function<const BackoffQueueEntry *,
                                  const BackoffQueueEntry *, bool> {
  bool operator()(const BackoffQueueEntry *first,
                  const BackoffQueueEntry *second) const {
...
std::size_t
MaxOrder(const util::FixedArray<util::stream::ChainPositions> &model) {
  std::size_t ret = 0;
  for (const util::stream::ChainPositions *m = model.begin(); m != model.end();
       ++m) {
...
public:
  explicit BackoffManager(
      const util::FixedArray<util::stream::ChainPositions> &models)
      : entered_(MaxOrder(models)), matrix_(models.size(), MaxOrder(models)),
        skip_write_(MaxOrder(models)) {
    std::size_t total = 0;
    for (const util::stream::ChainPositions *m = models.begin();
         m != models.end(); ++m) {
      total += m->size();
    }
    ...
    for (const util::stream::ChainPositions *m = models.begin();
         m != models.end(); ++m) {
      for (const util::stream::ChainPosition *j = m->begin(); j != m->end();
           ++j) {
        owner_.push_back(matrix_.Backoff(m - models.begin(), j - m->begin()),
                         *j);
        if (owner_.back()) {
  ...
  void Exit(std::size_t order_minus_1) {
    for (BackoffQueueEntry **i = entered_[order_minus_1].begin();
         i != entered_[order_minus_1].end(); ++i) {
      if ((*i)->Next())
        queue_.push(*i);
    }
...
private:
  ...
  bool TopMatches(const NGramHeader &header) const {
    return !queue_.empty() && (*queue_.top())->Order() == header.Order() &&
           std::equal(header.begin(), header.end(), (*queue_.top())->begin());
  }

  EntryOwner owner_;
  std::priority_queue<BackoffQueueEntry *, std::vector<BackoffQueueEntry *>,
                      PtrGreater>
      queue_;
...
class Recurse {
public:
  Recurse(const InterpolateInfo &info, // Must stay alive the entire time.
          std::size_t order, const util::stream::ChainPosition &merged_probs,
      ...
      : order_(order), encoding_(MakeEncoder(info, order)),
        input_(merged_probs,
               PartialProbGamma(order, encoding_.EncodedLength())),
        prob_out_(prob_out), backoff_out_(backoff_out), backoffs_(backoffs),
        lambdas_(&*info.lambdas.begin()), higher_(higher),
        decoded_backoffs_(info.Models()), extended_context_(order - 1) {
    // This is only for bigrams and above.  Summing unigrams is a much easier
    // case.
    assert(order >= 2);
  }
  ...
  void SameContext(const NGramHeader &context, Accum z_lower) {
    ...
    // This is the backoff term that applies when one assumes everything backs
    // off: \prod_i b_i(w_1^{n-1})^{\lambda_i}.
    ...
    std::size_t count = 0;
    for (;
         input_ && std::equal(context.begin(), context.end(), input_->begin());
         ++input_, ++prob_out_, ++count) {
      // Apply backoffs to probabilities.
      // TODO: change bounded sequence encoding to have an iterator for decoding
      ...
      for (std::size_t m = 0; m < NumModels(); ++m) {
        ...
        for (unsigned char backed_to = decoded_backoffs_[m];
             backed_to < order_ - 2; ++backed_to) {
        ...
        // Charge the backoff b(w_1^{n-1}) if applicable, but only to attain p(x
        // | w_1^{n-1})
      ...
      z_delta += pow(10.0, input_->Prob()) -
                 pow(10.0, input_->LowerProb() + backoff_once);

      // Write unnormalized probability record.
      std::copy(input_->begin(), input_->end(),
                reinterpret_cast<WordIndex *>(prob_out_.Get()));
  ...
  void ExtendContext(const NGramHeader &middle, Accum z_lower) {
    ...
    // Copy because the input will advance.  TODO avoid this copy by sharing
    // amongst classes.
    std::copy(middle.begin(), middle.end(), extended_context_.begin() + 1);
    while (input_ &&
           std::equal(middle.begin(), middle.end(), input_->begin() + 1)) {
      *extended_context_.begin() = *input_->begin();
      SameContext(NGramHeader(&*extended_context_.begin(), order_ - 1),
                  z_lower);
    }
  }
  ...
  // The BackoffManager class also injects backoffs when it skips ahead e.g.
  // b(</s>) = 1
  util::stream::Stream &BackoffStream() { return backoff_out_; }

private:
  // Write the probability to the correct place in prob_out_.  Should use a
  // proxy but currently incompatible with RewindableStream.
  float &ProbWrite() {
    return *reinterpret_cast<float *>(
        reinterpret_cast<uint8_t *>(prob_out_.Get()) +
        order_ * sizeof(WordIndex));
  }
...
class Thread {
public:
  Thread(const InterpolateInfo &info,
         util::FixedArray<util::stream::ChainPositions> &models_by_order,
         util::stream::Chains &prob_out, util::stream::Chains &backoff_out)
      : info_(info), models_by_order_(models_by_order), prob_out_(prob_out),
  ...
  void Run(const util::stream::ChainPositions &merged_probabilities) {
    ...
    ProxyStream<PartialProbGamma> in(merged_probabilities[0],
                                     PartialProbGamma(1, 0));
    ...
    // TODO HACK TODO: lmplz outputs p(<s>) = 1 to get q to compute nicely. That
    // will always result in 1.0 more than it should be.
    z -= 1.0;
    float log_z = log10(z);
    prob_write.Rewind();
    // Normalize unigram probabilities.
    for (WordIndex i = 0; i < count; ++i, ++prob_write) {
      *reinterpret_cast<float *>(reinterpret_cast<uint8_t *>(prob_write.Get()) +
                                 sizeof(WordIndex)) -= log_z;
    }
    prob_write.Poison();
    ...
    for (std::size_t order = max_order; order >= 2; --order) {
      higher_order.reset(new Recurse(
          info_, order, merged_probabilities[order - 1], prob_out_[order - 1],
          backoff_out_[order - 2], backoffs, higher_order.release()));
...
void Normalize(const InterpolateInfo &info,
               util::FixedArray<util::stream::ChainPositions> &models_by_order,
               util::stream::Chains &merged_probabilities,
               util::stream::Chains &prob_out,
               util::stream::Chains &backoff_out) {
