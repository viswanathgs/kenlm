...
void SetupInputs(std::size_t buffer_size, const UniversalVocab &vocab,
                 util::FixedArray<ModelBuffer> &models, bool exclude_highest,
                 util::FixedArray<util::stream::Chains> &chains,
                 util::FixedArray<util::stream::ChainPositions> &positions) {
  chains.clear();
  positions.clear();
  // TODO: much better memory sizing heuristics e.g. not making the chain larger
  // than it will use.
  ...
  for (std::size_t i = 0; i < models.size(); ++i) {
    ...
    for (std::size_t j = 0; j < models[i].Order() - exclude_highest; ++j) {
      config.entry_size =
          sizeof(WordIndex) * (j + 1) +
          sizeof(float) *
              2; // TODO do not include wasteful backoff for highest.
...
void SinkSort(const util::stream::SortConfig &config,
              util::stream::Chains &chains,
              util::stream::Sorts<Compare> &sorts) {
  for (std::size_t i = 0; i < chains.size(); ++i) {
...
template <class Compare>
void SourceSort(util::stream::Chains &chains,
                util::stream::Sorts<Compare> &sorts) {
...
void Pipeline(util::FixedArray<ModelBuffer> &models, const Config &config,
  ...
  SetupInputs(config.BufferSize(), *vocab, models, false, input_chains,
              models_by_order);

  util::stream::Chains merged_probs(max_order);
  for (std::size_t i = 0; i < max_order; ++i) {
    merged_probs.push_back(util::stream::ChainConfig(
        PartialProbGamma::TotalSize(info, i + 1), 2,
        config.BufferSize())); // TODO: not buffer_size
  }
  ...
  for (util::stream::Chains *i = input_chains.begin(); i != input_chains.end();
       ++i) {
  ...
  // Pass 2: normalize.
  {
    ...
    for (util::stream::Chains *i = input_chains.begin();
         i != input_chains.end(); ++i) {
      i->Wait(true);
    }
  ...
  SetupInputs(config.BufferSize(), *vocab, models, true, input_chains,
              models_by_order);
  util::stream::Chains probabilities(max_order), backoffs(max_order - 1);
  std::size_t block_count = 2;
  for (std::size_t i = 0; i < max_order; ++i) {
    // Careful accounting to ensure RewindableStream can fit the entire
    // vocabulary.
    ...
    std::size_t specify =
        std::max(config.BufferSize(), min_block * block_count);
    probabilities.push_back(util::stream::ChainConfig(
        NGram<float>::TotalSize(i + 1), block_count, specify));
  }
  for (std::size_t i = 0; i < max_order - 1; ++i) {
    backoffs.push_back(
        util::stream::ChainConfig(sizeof(float), 2, config.BufferSize()));
  }
  ...
  for (std::size_t i = 0; i < max_order - 1; ++i) {
    ...
    combined.push_back(util::stream::ChainConfig(
        NGram<ProbBackoff>::TotalSize(i + 1), 2, config.BufferSize()));
  }
