...
void Usage(const char *name, const char *default_mem) {
  std::cerr
      << "Usage: " << name
      << " [-u log10_unknown_probability] [-s] [-i] [-v] [-w mmap|after] [-p "
         "probing_multiplier] [-T trie_temporary] [-S trie_building_mem] [-q "
         "bits] [-b bits] [-a bits] [type] input.arpa [output.mmap]\n\n"
         "-u sets the log10 probability for <unk> if the ARPA file does not "
         "have one.\n"
         "   Default is -100.  The ARPA file will always take precedence.\n"
         "-s allows models to be built even if they do not have <s> and </s>.\n"
         "-i allows buggy models from IRSTLM by mapping positive log "
         "probability to 0.\n"
         "-v disables inclusion of the vocabulary in the binary file.\n"
         "-w mmap|after determines how writing is done.\n"
         "   mmap maps the binary file and writes to it.  Default for trie.\n"
         "   after allocates anonymous memory, builds, and writes.  Default "
         "for probing.\n"
         "-r \"order1.arpa order2 order3 order4\" adds lower-order rest costs "
         "from these\n"
         "   model files.  order1.arpa must be an ARPA file.  All others may "
         "be ARPA or\n"
         "   the same data structure as being built.  All files must have the "
         "same\n"
         "   vocabulary.  For probing, the unigrams must be in the same "
         "order.\n\n"
         "type is either probing or trie.  Default is probing.\n\n"
         "probing uses a probing hash table.  It is the fastest but uses the "
         "most memory.\n"
         "-p sets the space multiplier and must be >1.0.  The default is "
         "1.5.\n\n"
         "trie is a straightforward trie with bit-level packing.  It uses the "
         "least\n"
         "memory and is still faster than SRI or IRST.  Building the trie "
         "format uses an\n"
         "on-disk sort to save memory.\n"
         "-T is the temporary directory prefix.  Default is the output file "
         "name.\n"
         "-S determines memory use for sorting.  Default is "
      << default_mem
      << ".  This is compatible\n"
         "   with GNU sort.  The number is followed by a unit: \% for percent "
         "of physical\n"
         "   memory, b for bytes, K for Kilobytes, M for megabytes, then "
         "G,T,P,E,Z,Y.  \n"
         "   Default unit is K for Kilobytes.\n"
         "-q turns quantization on and sets the number of bits (e.g. -q 8).\n"
         "-b sets backoff quantization bits.  Requires -q and defaults to that "
         "value.\n"
         "-a compresses pointers using an array of offsets.  The parameter is "
         "the\n"
         "   maximum number of bits encoded by the array.  Memory is minimized "
         "subject\n"
         "   to the maximum, so pick 255 to minimize memory.\n\n"
         "-h print this help message.\n\n"
         "Get a memory estimate by passing an ARPA file without an output file "
         "name.\n";
  exit(1);
}
...
float ParseFloat(const char *from) {
  ...
  if (*end)
    throw util::ParseNumberException(from);
  return ret;
}
  ...
  try {
    ...
    while ((opt = getopt(argc, argv, "q:b:a:u:p:t:T:m:S:w:sir:vh")) != -1) {
      ...
      case 'q':
        ...
        if (!set_backoff_bits)
          config.backoff_bits = config.prob_bits;
        quantize = true;
        break;
      case 'b':
      ...
      case 'm': // legacy
        ...
        break;
      case 'S':
        config.building_memory = std::min(
    ...
    if (!strcmp(model_type, "probing")) {
      if (!set_write_method)
        config.write_method = Config::WRITE_AFTER;
      if (quantize || set_backoff_bits)
        ProbingQuantizationUnsupported();
      if (rest) {
