...
void PrintLead(const VocabReconstitute &vocab, ProxyStream<Payload> &stream,
               util::FileStream &out) {
...
void PrintARPA::Run(const util::stream::ChainPositions &positions) {
  ...
  for (unsigned order = 1; order < positions.size(); ++order) {
    out << "\\" << order << "-grams:" << '\n';
    for (ProxyStream<NGram<ProbBackoff> > stream(
             positions[order - 1], NGram<ProbBackoff>(NULL, order));
         stream; ++stream) {
  ...
  for (ProxyStream<NGram<Prob> > stream(positions.back(),
                                        NGram<Prob>(NULL, positions.size()));
       stream; ++stream) {
    PrintLead(vocab, stream, out);
    out << '\n';
  }
