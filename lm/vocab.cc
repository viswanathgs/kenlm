...
uint64_t HashForVocab(const char *str, std::size_t len) {
  // This proved faster than Boost's hash in speed trials: total load time
  // Murmur 67090000, Boost 72210000 Chose to use 64A instead of native so
  // binary format will be portable across 64 and 32 bit.
  return util::MurmurHash64A(str, len, 0);
}
} // namespace detail

namespace {
// Normally static initialization is a bad idea but MurmurHash is pure
// arithmetic, so this is ok.
...
void ReadWords(int fd, EnumerateVocab *enumerate, WordIndex expected_count,
  ...
  UTIL_THROW_IF(
      memcmp(check_unk, "<unk>", 6), FormatLoadException,
      "Vocabulary words are in the wrong place.  This could be because the "
      "binary file was built with stale gcc and old kenlm.  Stale gcc, "
      "including the gcc distributed with RedHat and OS X, has a bug that "
      "ignores pragma pack for template-dependent types.  New kenlm works "
      "around this, so you'll save memory but have to rebuild any binary files "
      "using the probing data structure.");
  if (!enumerate)
    return;
  enumerate->Add(0, "<unk>");
  ...
  UTIL_THROW_IF(expected_count != index, FormatLoadException,
                "The binary file has the wrong number of words at the end.  "
                "This could be caused by a truncated binary file.");
}
...
ImmediateWriteWordsWrapper::ImmediateWriteWordsWrapper(EnumerateVocab *inner,
                                                       int fd, uint64_t start)
...
SortedVocabulary::SortedVocabulary()
    : begin_(NULL), end_(NULL), enumerate_(NULL) {}
...
void SortedVocabulary::SetupMemory(void *start, std::size_t allocated,
                                   std::size_t entries, const Config &config) {
...
void SortedVocabulary::ConfigureEnumerate(EnumerateVocab *to,
...
struct RenumberEntry {
  ...
  const char *str;
  WordIndex old;
  bool operator<(const RenumberEntry &other) const { return hash < other.hash; }
};
#pragma pack(pop)
} // namespace

void SortedVocabulary::ComputeRenumbering(WordIndex types, int from_words,
                                          int to_words,
  ...
  UTIL_THROW_IF(memcmp(start, "<unk>", 6), FormatLoadException,
                "Vocab file does not begin with <unk> followed by null");
  std::vector<RenumberEntry> entries;
  entries.reserve(types - 1);
  RenumberEntry entry;
  entry.old = 1;
  for (entry.str = start + 6 /* skip <unk>\0 */; entry.str < start + file_size;
       ++entry.old) {
  ...
  UTIL_THROW_IF2(entries.size() != types - 1, "Wrong number of vocab ids.  Got "
                                                  << (entries.size() + 1)
                                                  << " expected " << types);
  ...
  // Write out new vocab file.
  {
    ...
    out << "<unk>" << '\0';
    for (std::vector<RenumberEntry>::const_iterator i = entries.begin();
         i != entries.end(); ++i) {
      out << i->str << '\0';
    }
  }
...
void SortedVocabulary::LoadedBinary(bool have_words, int fd, EnumerateVocab *to,
  ...
  bound_ = end_ - begin_ + 1;
  if (have_words)
    ReadWords(fd, to, bound_, offset);
}

template <class T> void SortedVocabulary::GenericFinished(T *reorder) {
  if (enumerate_) {
    if (!strings_to_enumerate_.empty()) {
      util::PairedIterator<T *, StringPiece *> values(
          reorder + 1, &*strings_to_enumerate_.begin());
...
uint64_t ProbingVocabulary::Size(uint64_t entries, float probing_multiplier) {
  return ALIGN8(sizeof(detail::ProbingVocabularyHeader)) +
         Lookup::Size(entries, probing_multiplier);
}
...
void ProbingVocabulary::SetupMemory(void *start, std::size_t allocated) {
  ...
  lookup_ = Lookup(static_cast<uint8_t *>(start) +
                       ALIGN8(sizeof(detail::ProbingVocabularyHeader)),
                   allocated);
  bound_ = 1;
  saw_unk_ = false;
}
...
void ProbingVocabulary::Relocate(void *new_start) {
  ...
  lookup_.Relocate(static_cast<uint8_t *>(new_start) +
                   ALIGN8(sizeof(detail::ProbingVocabularyHeader)));
}

void ProbingVocabulary::ConfigureEnumerate(EnumerateVocab *to,
                                           std::size_t /*max_entries*/) {
  enumerate_ = to;
  if (enumerate_) {
    enumerate_->Add(0, "<unk>");
...
void ProbingVocabulary::LoadedBinary(bool have_words, int fd,
                                     EnumerateVocab *to, uint64_t offset) {
  UTIL_THROW_IF(header_->version != kProbingVocabularyVersion,
                FormatLoadException,
                "The binary file has probing version "
                    << header_->version << " but the code expects version "
                    << kProbingVocabularyVersion
                    << ".  Please rerun build_binary using the same version of "
                       "the code.");
  ...
  case COMPLAIN:
    if (config.messages)
      *config.messages
          ...
          << config.unknown_missing_logprob << "." << std::endl;
    break;
  case THROW_UP:
    UTIL_THROW(SpecialWordMissingException,
               "The ARPA file is missing <unk> and the model is configured to "
               "throw an exception.");
  }
}
...
void MissingSentenceMarker(const Config &config, const char *str) {
  ...
  case COMPLAIN:
    if (config.messages)
      *config.messages << "Missing special word " << str
                       << "; will treat it as <unk>.";
    break;
  case THROW_UP:
    UTIL_THROW(SpecialWordMissingException,
               "The ARPA file is missing "
                   << str
                   << " and the model is configured to reject these models.  "
                      "Run build_binary -s to disable this check.");
  }
}
