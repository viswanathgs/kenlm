...
#include <windows.h>
#warning                                                                       \
    "The file functions on MinGW have not been tested for file sizes above 2^31 - 1.  Please read https://stackoverflow.com/questions/12539488/determine-64-bit-file-size-in-c-on-mingw-32-bit and fix"
...
FDException::FDException(int fd) throw()
    : fd_(fd), name_guess_(NameFromFD(fd)) {
  *this << "in " << name_guess_ << ' ';
}

FDException::~FDException() throw() {}

EndOfFileException::EndOfFileException() throw() { *this << "End of file"; }
...
int OpenReadOrThrow(const char *name) {
  ...
  UTIL_THROW_IF(-1 == (ret = _open(name, _O_BINARY | _O_RDONLY)),
                ErrnoException, "while opening " << name);
#else
  UTIL_THROW_IF(-1 == (ret = open(name, O_RDONLY)), ErrnoException,
                "while opening " << name);
#endif
  return ret;
}
...
#if defined(_WIN32) || defined(_WIN64)
  UTIL_THROW_IF(
      -1 == (ret = _open(name, _O_CREAT | _O_TRUNC | _O_RDWR | _O_BINARY,
                         _S_IREAD | _S_IWRITE)),
      ErrnoException, "while creating " << name);
#else
  UTIL_THROW_IF(-1 == (ret = open(name, O_CREAT | O_TRUNC | O_RDWR,
...
uint64_t SizeFile(int fd) {
  ...
  int ret = fstat(fd, &sb);
  if (ret == -1 || (!sb.st_size && !S_ISREG(sb.st_mode)))
    return kBadSize;
  return sb.st_size;
#elif defined(_WIN32) || defined(_WIN64)
...
#endif
      (fd, to);
  UTIL_THROW_IF_ARG(ret, FDException, (fd),
                    "while resizing to " << to << " bytes");
}
...
void HolePunch(int fd, uint64_t offset, uint64_t size) {
  ...
  UTIL_THROW_IF_ARG(
      -1 == fallocate(fd, FALLOC_FL_PUNCH_HOLE | FALLOC_FL_KEEP_SIZE, offset,
                      size),
      FDException, (fd),
      "in punching a hole at " << offset << " for " << size << " bytes.");
#else
  UTIL_THROW(UnsupportedOSException,
             "fallocate hole punching requires Linux and glibc >= 2.18");
#endif
}
...
std::size_t GuardLarge(std::size_t size) {
  ...
  // building with larger files, so APPLE is also here.
    ...
    defined(OS_ANDROID) || defined(__MINGW32__)
...
std::size_t PartialRead(int fd, void *to, std::size_t amount) {
  ...
  DWORD larger_size =
      static_cast<DWORD>(std::min<std::size_t>(kMaxDWORD, amount));
  DWORD smaller_size =
      28672; // Received reports that 31346 worked but higher values did not.
             // This rounds down to the nearest multiple of 4096, the page size.
  if (!ReadFile(file_handle, to, larger_size, &ret, NULL)) {
    DWORD last_error = GetLastError();
    if (last_error != ERROR_NOT_ENOUGH_MEMORY ||
        !ReadFile(file_handle, to, smaller_size, &ret, NULL)) {
  ...
  UTIL_THROW_IF_ARG(ret < 0, FDException, (fd),
                    "while reading " << amount << " bytes");
#endif
...
void ReadOrThrow(int fd, void *to_void, std::size_t amount) {
  ...
  while (amount) {
    ...
    UTIL_THROW_IF(ret == 0, EndOfFileException,
                  " in " << NameFromFD(fd) << " but there should be " << amount
                         << " more bytes to read.");
    amount -= ret;
    to += ret;
  }
...
void WriteOrThrow(int fd, const void *data_void, std::size_t size) {
  ...
  while (size) {
    ...
    UTIL_THROW_IF_ARG(ret < 1, FDException, (fd),
                      "while writing " << size << " bytes");
    data += ret;
    size -= ret;
  }
}
...
void WriteOrThrow(FILE *to, const void *data, std::size_t size) {
  if (!size)
    return;
  UTIL_THROW_IF(1 != std::fwrite(data, size, 1, to), ErrnoException,
                "Short write; requested size " << size);
}
...
void ErsatzPRead(int fd, void *to_void, std::size_t size, uint64_t off) {
  ...
  while (size) {
#if defined(_WIN32) || defined(_WIN64)
    /* BROKEN: changes file pointer.  Even if you save it and change it back, it
     * won't be safe to use concurrently with write() or read() which lmplz
     * does. */
    ...
    UTIL_THROW_IF(
        !ReadFile((HANDLE)_get_osfhandle(fd), to, reading, &ret, &overlapped),
        WindowsException, "ReadFile failed for offset " << off);
#else
    ssize_t ret;
    errno = 0;
    ret =
    ...
    if (ret <= 0) {
      if (ret == -1 && errno == EINTR)
        continue;
      UTIL_THROW_IF(ret == 0, EndOfFileException,
                    " for reading " << size << " bytes at " << off << " from "
                                    << NameFromFD(fd));
      UTIL_THROW_ARG(FDException, (fd),
                     "while reading " << size << " bytes at offset " << off);
    }
#endif
    size -= ret;
    off += ret;
...
void ErsatzPWrite(int fd, const void *from_void, std::size_t size,
                  uint64_t off) {
  const uint8_t *from = static_cast<const uint8_t *>(from_void);
  while (size) {
#if defined(_WIN32) || defined(_WIN64)
    /* Changes file pointer.  Even if you save it and change it back, it won't
     * be safe to use concurrently with write() or read() */
    // size_t might be 64-bit.  DWORD is always 32.
    ...
    overlapped.Offset = static_cast<DWORD>(off);
    overlapped.OffsetHigh = static_cast<DWORD>(off >> 32);
    UTIL_THROW_IF(!WriteFile((HANDLE)_get_osfhandle(fd), from, writing, &ret,
                             &overlapped),
                  Exception, "WriteFile failed for offset " << off);
#else
    ssize_t ret;
    errno = 0;
    ret =
    ...
    if (ret <= 0) {
      if (ret == -1 && errno == EINTR)
        continue;
      UTIL_THROW_IF(ret == 0, EndOfFileException,
                    " for writing " << size << " bytes at " << off << " from "
                                    << NameFromFD(fd));
      UTIL_THROW_ARG(FDException, (fd),
                     "while writing " << size << " bytes at offset " << off);
    }
#endif
    size -= ret;
    off += ret;
...
uint64_t InternalSeek(int fd, int64_t off, int whence) {
  ...
  Offset ret = lseek(fd, off, whence);
#endif
  UTIL_THROW_IF_ARG((Offset)-1 == ret, FDException, (fd),
                    "while seeking to " << off << " whence " << whence);
  return (uint64_t)ret;
}
} // namespace
...
uint64_t SeekEnd(int fd) { return InternalSeek(fd, 0, SEEK_END); }

std::FILE *FDOpenOrThrow(scoped_fd &file) {
  std::FILE *ret = fdopen(file.get(), "r+b");
  UTIL_THROW_IF_ARG(!ret, FDException, (file.get()),
                    "Could not fdopen for write");
  file.release();
  return ret;
}
...
int MakeTemp(const StringPiece &base) {
  ...
  name.push_back(0);
  int ret;
  UTIL_THROW_IF(-1 == (ret = mkstemp_and_unlink(&name[0])), ErrnoException,
...
int DupOrThrow(int fd) {
  int ret = dup(fd);
  UTIL_THROW_IF_ARG(ret == -1, FDException, (fd),
                    "in duplicating the file descriptor");
  return ret;
}
...
bool TryName(int fd, std::string &out) {
  ...
  // lstat gave us a size, but I've seen it grow, possibly due to symlinks on
  // top of symlinks.
  while (true) {
