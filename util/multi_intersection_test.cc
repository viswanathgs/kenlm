...
BOOST_AUTO_TEST_CASE(Empty) {
  ...
  sets.push_back(boost::iterator_range<const unsigned int *>(
...
BOOST_AUTO_TEST_CASE(Single) {
  ...
  nums.push_back(4);
  nums.push_back(100);
  std::vector<boost::iterator_range<std::vector<unsigned int>::const_iterator> >
      sets;
  sets.push_back(nums);
...
boost::iterator_range<const T *> RangeFromArray(const T (&arr)[len]) {
