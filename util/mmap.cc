...
std::size_t RoundUpSize(const scoped_memory &mem) {
  ...
  case scoped_memory::MMAP_ROUND_PAGE_ALLOCATED:
    return RoundUpPow2<std::size_t>(mem.size(),
                                    static_cast<std::size_t>(SizePage()));
  default:
    return mem.size();
  }
}
...
scoped_memory::scoped_memory(std::size_t size, bool zeroed)
    : data_(NULL), size_(0), source_(NONE_ALLOCATED) {
...
void *MapOrThrow(std::size_t size, bool for_write, int flags, bool prefault,
  ...
  HANDLE hMapping =
      CreateFileMapping((HANDLE)_get_osfhandle(fd), NULL, protectC,
  ...
  void *ret;
  UTIL_THROW_IF((ret = mmap(NULL, size, protect, flags, fd, offset)) ==
                    MAP_FAILED,
                ErrnoException,
                "mmap failed for size " << size << " at offset " << offset);
#ifdef MADV_HUGEPAGE
...
#if defined(_WIN32) || defined(_WIN64)
  UTIL_THROW_IF(!::FlushViewOfFile(start, length), ErrnoException,
                "Failed to sync mmap");
#else
  UTIL_THROW_IF(length && msync(start, length, MS_SYNC), ErrnoException,
                "Failed to sync mmap");
#endif
}
...
#if defined(_WIN32) || defined(_WIN64)
  UTIL_THROW_IF(!::UnmapViewOfFile(start), ErrnoException,
                "Failed to unmap a file");
#else
  UTIL_THROW_IF(munmap(start, length), ErrnoException,
                "munmap failed with " << start << " for length " << length);
#endif
}
...
bool TryHuge(std::size_t size, bool populate, uint8_t alignment_bits,
             scoped_memory::Alloc huge_scheme, scoped_memory &to) {
  ...
  int flags =
      ...
      (alignment_bits
       << 26 /* This is MAP_HUGE_SHIFT but some headers are too old. */);
  if (populate)
    flags |= MAP_POPULATE;
  ...
  // Round up requested size to multiple of page size.  This will allow the
  // pages after to be munmapped.
  ...
  scoped_mmap larger(mmap(NULL, ask, PROT_READ | PROT_WRITE,
                          MAP_PRIVATE | MAP_ANONYMOUS, -1, 0),
                     ask);
  if (larger.get() == MAP_FAILED)
    return false;

  // Throw out pages before the alignment point.
  uintptr_t base = reinterpret_cast<uintptr_t>(larger.get());
  // Round up to next multiple of alignment.
  uintptr_t rounded_up =
      RoundUpPow2(base, static_cast<uintptr_t>(1) << alignment_bits);
  if (base != rounded_up) {
    // If this throws an exception (which it shouldn't) then we want to unmap
    // the whole thing by keeping it in larger.
    UnmapOrThrow(larger.get(), rounded_up - base);
    larger.steal();
    larger.reset(reinterpret_cast<void *>(rounded_up),
                 ask - (rounded_up - base));
  }
  ...
  if (larger.size() > size_up) {
    ...
    UnmapOrThrow(static_cast<uint8_t *>(larger.get()) + size_up,
                 larger.size() - size_up);
...
void HugeMalloc(std::size_t size, bool zeroed, scoped_memory &to) {
  ...
  if (size >= (1ULL << 30) &&
      TryHuge(size, zeroed, 30, scoped_memory::MMAP_ROUND_1G_ALLOCATED, to))
    return;
  // Attempt 2 MB pages.
  if (size >= (1ULL << 21) &&
      TryHuge(size, zeroed, 21, scoped_memory::MMAP_ROUND_2M_ALLOCATED, to))
    return;
#endif // __linux__
  // Non-linux will always do this, as will small allocations on Linux.
  to.reset(zeroed ? calloc(1, size) : malloc(size), size,
           scoped_memory::MALLOC_ALLOCATED);
  UTIL_THROW_IF(!to.get(), ErrnoException,
                "Failed to allocate " << size << " bytes");
}

namespace {
#ifdef __linux__
const std::size_t kTransitionHuge =
    std::max<std::size_t>(1ULL << 21, SizePage());
#endif // __linux__
...
void HugeRealloc(std::size_t to, bool zero_new, scoped_memory &mem) {
  ...
  case scoped_memory::MMAP_ALLOCATED:
    ...
    if (to <= SizePage()) {
      ...
      if (zero_new && to > mem.size())
        memset(static_cast<uint8_t *>(replacement.get()) + mem.size(), 0,
               to - mem.size());
  ...
  case scoped_memory::MALLOC_ALLOCATED:
#ifdef __linux__
    // Transition larger allocations to huge pages, but don't keep trying if
    // we're still malloc allocated.
...
#endif // __linux__
    {
      ...
      UTIL_THROW_IF(!new_addr, ErrnoException,
                    "realloc to " << to << " bytes failed.");
      if (zero_new && to > mem.size())
        memset(static_cast<uint8_t *>(new_addr) + mem.size(), 0,
               to - mem.size());
      mem.steal();
...
void MapRead(LoadMethod method, int fd, uint64_t offset, std::size_t size,
             scoped_memory &out) {
  switch (method) {
  case LAZY:
    out.reset(MapOrThrow(size, false, kFileFlags, false, fd, offset), size,
              scoped_memory::MMAP_ALLOCATED);
    break;
  case POPULATE_OR_LAZY:
#ifdef MAP_POPULATE
  case POPULATE_OR_READ:
#endif
    out.reset(MapOrThrow(size, false, kFileFlags, true, fd, offset), size,
              scoped_memory::MMAP_ALLOCATED);
    break;
#ifndef MAP_POPULATE
  case POPULATE_OR_READ:
...
Rolling::Rolling(int fd, bool for_write, std::size_t block,
                 std::size_t read_bound, uint64_t offset, uint64_t amount) {
  current_begin_ = 0;
  current_end_ = 0;
  fd_ = fd;
  file_begin_ = offset;
...
void *Rolling::ExtractNonRolling(scoped_memory &out, uint64_t index,
                                 std::size_t size) {
  out.reset();
  if (IsPassthrough())
    return static_cast<uint8_t *>(get()) + index;
  ...
  out.reset(
      MapOrThrow(map_size, for_write_, kFileFlags, true, fd_, offset - cruft),
      map_size, scoped_memory::MMAP_ALLOCATED);
