...
void BitPackingSanity() {
  ...
  if ((neg1.i ^ pos1.i) != 0x80000000)
    UTIL_THROW(Exception, "Sign bit is not 0x80000000");
  char mem[57 + 8];
  ...
  for (uint64_t b = 0; b < 57 * 8; b += 57) {
    if (test57 != ReadInt57(mem, b, 57, (1ULL << 57) - 1))
      UTIL_THROW(Exception, "The bit packing routines are failing for your "
                            "architecture.  Please send a bug report with your "
