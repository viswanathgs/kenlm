...
ReadBase *ReadBase::Current(ReadCompressed &thunk) {
  return thunk.internal_.get();
}
...
ReadBase *ReadFactory(int fd, uint64_t &raw_amount, const void *already_data,
                      std::size_t already_size, bool require_compressed);

// Completed file that other classes can thunk to.
class Complete : public ReadBase {
public:
  std::size_t Read(void *, std::size_t, ReadCompressed &) { return 0; }
};
...
public:
  UncompressedWithHeader(int fd, const void *already_data,
                         std::size_t already_size)
      : fd_(fd) {
    assert(already_size);
...
public:
  StreamCompressed(int fd, const void *already_data, std::size_t already_size)
      : file_(fd), in_buffer_(MallocOrThrow(kInputBuffer)),
        back_(memcpy(in_buffer_.get(), already_data, already_size),
              already_size) {}
  ...
  std::size_t Read(void *to, std::size_t amount, ReadCompressed &thunk) {
    ...
    do {
      ...
      if (!back_.Process()) {
        ...
        std::size_t ret = static_cast<const uint8_t *>(
                              static_cast<void *>(back_.Stream().next_out)) -
                          static_cast<const uint8_t *>(to);
        ReplaceThis(ReadFactory(file_.release(), ReadCount(thunk),
                                back_.Stream().next_in, back_.Stream().avail_in,
                                true),
                    thunk);
        if (ret)
          return ret;
        // We did not read anything this round, so clients might think EOF.
        // Transfer responsibility to the next reader.
...
public:
  GZip(const void *base, std::size_t amount) {
    ...
    UTIL_THROW_IF(Z_OK != inflateInit2(&stream_, 32 + 15), GZException,
  ...
  void SetOutput(void *to, std::size_t amount) {
    ...
    stream_.avail_out =
        std::min<std::size_t>(std::numeric_limits<uInt>::max(), amount);
  }
  ...
  bool Process() {
    ...
    default:
      UTIL_THROW(GZException, "zlib encountered "
                                  << (stream_.msg ? stream_.msg : "an error ")
                                  << " code " << result);
    }
  }
...
public:
  ...
  void SetOutput(void *base, std::size_t amount) {
    ...
    stream_.avail_out =
        std::min<std::size_t>(std::numeric_limits<unsigned int>::max(), amount);
  }
...
private:
  void HandleError(int value) {
    ...
    case BZ_DATA_ERROR_MAGIC:
      UTIL_THROW(BZException, "bzip2 detected bad magic bytes.  Perhaps this "
                              "was not a bzip2 file after all?");
    case BZ_MEM_ERROR:
      throw std::bad_alloc();
    default:
...
class XZip {
public:
  XZip(const void *base, std::size_t amount) : stream_(), action_(LZMA_RUN) {
    memset(&stream_, 0, sizeof(stream_));
    SetInput(base, amount);
    HandleError(lzma_stream_decoder(&stream_, UINT64_MAX, 0));
  }

  ~XZip() { lzma_end(&stream_); }
...
enum MagicResult { UTIL_UNKNOWN, UTIL_GZIP, UTIL_BZIP, UTIL_XZIP };
...
MagicResult DetectMagic(const void *from_void, std::size_t length) {
  ...
  if (length >= sizeof(kBZMagic) &&
      !memcmp(header, kBZMagic, sizeof(kBZMagic))) {
    return UTIL_BZIP;
  }
  const uint8_t kXZMagic[6] = {0xFD, '7', 'z', 'X', 'Z', 0x00};
  if (length >= sizeof(kXZMagic) &&
      !memcmp(header, kXZMagic, sizeof(kXZMagic))) {
    return UTIL_XZIP;
  }
  return UTIL_UNKNOWN;
}
...
ReadBase *ReadFactory(int fd, uint64_t &raw_amount, const void *already_data,
                      const std::size_t already_size, bool require_compressed) {
  scoped_fd hold(fd);
  std::string header(reinterpret_cast<const char *>(already_data),
                     already_size);
  if (header.size() < ReadCompressed::kMagicSize) {
    std::size_t original = header.size();
    header.resize(ReadCompressed::kMagicSize);
    std::size_t got =
        ReadOrEOF(fd, &header[original], ReadCompressed::kMagicSize - original);
    raw_amount += got;
  ...
  case UTIL_GZIP:
#ifdef HAVE_ZLIB
    return new StreamCompressed<GZip>(hold.release(), header.data(),
                                      header.size());
#else
    UTIL_THROW(
        CompressedException,
        "This looks like a gzip file but gzip support was not compiled in.");
#endif
  case UTIL_BZIP:
#ifdef HAVE_BZLIB
    return new StreamCompressed<BZip>(hold.release(), &header[0],
                                      header.size());
#else
    UTIL_THROW(CompressedException,
               "This looks like a bzip file (it begins with BZh), but bzip "
               "support was not compiled in.");
#endif
  case UTIL_XZIP:
#ifdef HAVE_XZLIB
    return new StreamCompressed<XZip>(hold.release(), header.data(),
                                      header.size());
#else
    UTIL_THROW(
        CompressedException,
        "This looks like an xz file, but xz support was not compiled in.");
#endif
  default:
    UTIL_THROW_IF(require_compressed, CompressedException,
                  "Uncompressed data detected after a compresssed file.  This "
                  "could be supported but usually indicates an error.");
    return new UncompressedWithHeader(hold.release(), header.data(),
                                      header.size());
  }
}
...
ReadCompressed::ReadCompressed(int fd) { Reset(fd); }

ReadCompressed::ReadCompressed(std::istream &in) { Reset(in); }
